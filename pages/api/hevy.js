...
export default async function handler(req, res) {
  ...
  if (!HEVY_API_KEY) return res.status(500).json({ error: "HEVY_API_KEY environment variable is not set" })
  try {
    if (endpoint === "workouts") {
      const data = await fetchHevy("/workouts?page=1&pageSize=10")
      res.json(data)
    } else if (endpoint === "routines") {
      const data = await fetchHevy("/routines?page=1&pageSize=10")
      res.json(data)
    } else if (endpoint === "exercise_templates") {
      const data = await fetchHevy("/exercise_templates?page=1&pageSize=10")
      res.json(data)
    } else {
